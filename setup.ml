...
(* OASIS_START *)
(* DO NOT EDIT (digest: 73be9177706084dac6f24b4fa608f86c) *)
(*
...
# 22 "/home/maurer/Sources/live/oasis/src/plugins/custom/CustomPlugin.ml"
...
let setup_t =
  {
          ...
          [
            ...
            Executable
              ({
                ...
                {exec_custom = false; exec_main_is = "test_mc.ml"});
            Test
              ({
                cs_name = "test_mc";
                cs_data = PropList.Data.create ();
    ...
    oasis_version = "0.4.5";
    oasis_digest =
      Some "7\242\175\144\192\191\012\b\249u\201\001\180Q\231\242";
    oasis_exec = None;
    oasis_setup_args = [];
    setup_update = false
  };;
...
let def_llvm_flags () =
  let llvm_config = BaseCheck.prog_best "llvm_config" ["llvm-config"] () in
  ["cflags"; "ldflags"; "libs"]
  |> List.map begin
    fun opt ->
      BaseEnv.var_define ("llvm_" ^ opt)
        (fun () -> exec llvm_config ["--" ^ opt])
  end
  |> ignore

let os_type =
  match Sys.os_type with
  | "Unix"  -> (match BaseStandardVar.system () with
      | "macosx" -> `Mac
      | "linux"  -> `Linux
      | _        -> `OtherUnix)
  | "Win32" -> `Windows
...
let def_sys_libs () =
  BaseEnv.var_define "sys_libs" begin
    fun () -> match os_type with
      | `Linux   -> "-lz -lcurses -lstdc++ -ldl"
      | `Windows -> "C:/MinGW/bin/libstdc++-6.dll -lpsapi -limagehlp"
      | `Mac     -> "-lz -lcurses -lc++ -ldl"
      | _        -> ""
  end
  |> ignore
